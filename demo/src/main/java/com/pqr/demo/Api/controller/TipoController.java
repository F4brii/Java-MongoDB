...
public class TipoController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Tipo> ObtenerTipo(@PathVariable String id) {
        try {
            return new ResponseEntity<Tipo>(useCase.ObtenerTipo(id), HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<Tipo>(useCase.ObtenerTipo(id), HttpStatus.BAD_REQUEST);
        }
    }
}

...
public class UsuarioController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Usuario> ObtenerUsuarioId(@PathVariable String id) {
        try {
            return new ResponseEntity<Usuario>(useCase.ObtenerUsuarioId(id), HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<Usuario>(useCase.ObtenerUsuarioId(id), HttpStatus.BAD_REQUEST);
        }
    }
}
